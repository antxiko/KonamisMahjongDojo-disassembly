...
# Aqui el origen es OTRO JUEGO: Athletic Land, el mismo armazon B de Konami,
# desensamblado al 100 % y comentado. No es el mismo programa, asi que el mapa
# solo casa los tramos donde el codigo coincide de verdad; lo demas se comenta
# a mano. Se pueden pasar por la linea de ordenes:
#     python3 tools/porta_notas.py mapa [origen.asm] [destino.asm]
ORIGEN = '../ATHLETIC_DISAM/src/athletic.asm'
DESTINO = 'src/mahjong.asm'
if sys.argv[1:2] == ['mapa'] and len(sys.argv) > 3:
    ORIGEN, DESTINO = sys.argv[2], sys.argv[3]
